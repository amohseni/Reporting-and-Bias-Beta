...
for (i in 1:TotalDataPoints) {
  ...
  for (r in 1:Runs) {
    # Sample (10^q) new reports from the News Distribution to be observed by the agent
    SampleOfNewsReports <-
      sample(x,
    ...
    # Save the beliefs to the data vector
    meanPerceptionVec[r] <- meanPerception
    sdPerceptionVec[r] <- sdPerception
  }
  # Save the average of the runs inot the data frame
  ...
  for (j in 1:length(BiasVEC)) {
    ...
    for (k in 1:length(BiasStrengthVEC)) {
      ...
      for (l in 1:length(FairAndBalancedVEC)) {
        ...
        pdf(paste("MeanMSE+", "Mu=", Mu, "+b=", b, "+s=", s, "+FaB=", FaB, ".pdf", sep = ""))
        G <-
  ...
  for (j in 1:length(BiasVEC)) {
    ...
    for (k in 1:length(BiasStrengthVEC)) {
      ...
      for (l in 1:length(FairAndBalancedVEC)) {
        ...
        # Having set a given combinatio of parameters,
        # open a PDF in which to save the plot
        pdf(paste("VarMSE+", "Mu=", Mu, "+b=", b, "+s=", s, "+FaB=", FaB, ".pdf", sep = ""))
        G <-
          ggplot(data = Df[Df$TrueMean == Mu &
                             # Create the plot for a given combinatio of parameters,
                             Df$Bias == b &
